...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.memberSubscription.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.member-subscriptions.update", [$memberSubscription->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('member_email') ? 'is-invalid' : '' }}" name="member_email_id" id="member_email_id" required>
                    @foreach($member_emails as $id => $entry)
                        <option value="{{ $id }}" {{ (old('member_email_id') ? old('member_email_id') : $memberSubscription->member_email->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subscription_name') ? 'is-invalid' : '' }}" name="subscription_name_id" id="subscription_name_id" required>
                    @foreach($subscription_names as $id => $entry)
                        <option value="{{ $id }}" {{ (old('subscription_name_id') ? old('subscription_name_id') : $memberSubscription->subscription_name->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" name="payment_method" id="payment_method" required>
                    ...
                    @foreach(App\Models\MemberSubscription::PAYMENT_METHOD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payment_method', $memberSubscription->payment_method) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $memberSubscription->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\MemberSubscription::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $memberSubscription->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('expiry_date') ? 'is-invalid' : '' }}" type="text" name="expiry_date" id="expiry_date" value="{{ old('expiry_date', $memberSubscription->expiry_date) }}">
